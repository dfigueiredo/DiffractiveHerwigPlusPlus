...
class UEAnalyzer: public edm::EDAnalyzer {
public:
  /// Constructor
  UEAnalyzer(const edm::ParameterSet& pset);
...
/// Constructor
UEAnalyzer::UEAnalyzer(const edm::ParameterSet& pset)
{
...
void UEAnalyzer::endJob(){
  hEnergyvsEta->Scale(1/(float)nevents);	
}

void UEAnalyzer::analyze(const edm::Event & ev, const edm::EventSetup&){
  nevents++; 
